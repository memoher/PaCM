(function () {
	...
	PaCM.services.factory('dbInstaller', function (settings, dbContext) {

        var debugMode = 1;

        var dbVersion = settings.serverUrl + '_2016.07.10.10.00';
		...
		var installDatabase = function (onSuccess, onError, debugMode) {
            ...
            var fnc03 = function (tx) {
                ...
                var sqlCommands = [
				    ...
				    'create table MntCellsReviews ( Id TEXT not null, Voltage NUMERIC, WithVoltage BOOL not null, Density NUMERIC, WithDensity BOOL not null, Comments TEXT, CreatedOn DATETIME not null, LastModified DATETIME not null, ReplicationStatus BOOL not null, CellId TEXT not null, MaintenanceId TEXT, AssemblyId TEXT, primary key (Id), constraint FK_CellReview_CellId foreign key (CellId) references MntCells, constraint FK_CellReview_MaintenanceId foreign key (MaintenanceId) references MntMaintenances, constraint FK_CellReview_AssemblyId foreign key (AssemblyId) references MntAssemblies )',
